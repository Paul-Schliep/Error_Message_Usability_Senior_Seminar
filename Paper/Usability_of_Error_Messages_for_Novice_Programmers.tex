...
In section 3, we discuss a study performed by Marceau et al that analyzes the error messages in the DrRacket integrated development environment~\cite{Marceau:2011:MEE:1953163.1953308}.
...
Racket is a dynamically typed language, which means a type is interpreted at runtime rather than compile time and objects in the language are not bound to types.
Thus it is possible in Racket to bind a name to objects of different types during the execution of a program since every variable name is bound only to an object.
...
However, C++ and Java are occasionally taught in a first computer science course.
...
Figure 1 shows the results of Marceau et al study. 
...
\todo[inline, color=red]{having a tough time summarizing paper}
...
Rather, simplified vocabulary would be better suited at entry-level levels in DrRacket, and the higher levels later in the course would help students become familiar with the vocabulary.
...
Marceau et al created a series of recommendations that they have implemented in HTDP libraries for DrRacket, which requires further research to find the effectiveness this has for introductory students.~\cite{htdp-teachpacks}.
